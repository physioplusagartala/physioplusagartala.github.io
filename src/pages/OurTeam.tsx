...
const teamMembers = [
  ...
  },
  {
    ...
    experience: "6 Years of Experience",
    highlights: [
      "Specialist in Orthopedic Physiotherapy",
    ],
  },
  ...
  <Layout>
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-6xl px-4 py-16 md:py-24 text-center">
        ...
        <p className="mt-3 text-primary-foreground/70 md:text-lg max-w-2xl mx-auto">
          Our team of qualified physiotherapists brings years of clinical expertise to help you recover faster.
        </p>
      </div>
    </section>
    ...
    <section className="section-padding">
      <div className="container-narrow">
        <SectionHeading title="Our Physiotherapists" subtitle="Experienced professionals dedicated to your recovery." />
        <div className="grid gap-8 md:grid-cols-2">
          {teamMembers.map((member) => (
            <Card key={member.name} className="border-border hover:shadow-xl transition-shadow overflow-hidden">
              <CardContent className="p-0">
                {/* Colored header strip */}
                <div className="bg-primary px-6 py-5">
                  <h3 className="font-heading text-xl font-bold text-primary-foreground">{member.name}</h3>
                  <p className="mt-1 text-sm font-medium text-primary-foreground/80">{member.credentials}</p>
                </div>
                <div className="p-6 space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-heading text-sm font-semibold text-foreground">{member.role}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary/10 shrink-0">
                      <Briefcase className="h-4.5 w-4.5 text-primary" />
                    </div>
                    <span className="text-sm font-medium text-foreground">{member.experience}</span>
                  </div>
